...
def register_chapters() -> list[Callable[[ManualPDF], None]]:
    ...
    def ch_sales(p: ManualPDF) -> None:
        ...
        p.bullet("يظهر المتبقي في المستودع تلقائياً؛ الدفعات تُخصم بنظام FIFO دون إدخال رقم دفعة.")
    ...
    def ch_warehouse(p: ManualPDF) -> None:
        ...
        p.bullet("عند البيع: لا يُطلب رقم الدفعة؛ النظام يخصم تلقائياً بنظام FIFO من المخزون.")
